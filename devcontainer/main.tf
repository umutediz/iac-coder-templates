...
variable "namespace" {
  ...
  description = "Kubernetes namespace where workspace resources are created."
}

variable "storage_class" {
  type        = string
  default     = "nfs-fast-annotated"
  description = "StorageClass for persistent workspace directories."
}

variable "cache_repo" {
  default     = "coder-image-cache.coder.svc.cluster.local:5000/coder-devcontainer-cache"
    ...
    template {
      metadata {
        labels = {
          ...
          "coder.com/user-name"        = data.coder_workspace_owner.me.name
        }
      }
      spec {
        security_context {}
